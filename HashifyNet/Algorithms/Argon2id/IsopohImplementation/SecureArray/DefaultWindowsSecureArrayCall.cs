...
namespace Isopoh.Cryptography.SecureArray
{
	...
	/// </summary>
	internal class DefaultWindowsSecureArrayCall : SecureArrayCall
	{
						...
						if (!is32BitSubsystem.HasValue)
						{
							...
							else
							{
								...
								else
								{
									...
									{
										is32BitSubsystem =
											UnsafeNativeMethods.IsWow64Process(UnsafeNativeMethods.GetCurrentProcess(), out var isWow64Process)
											&& isWow64Process;
									}
								}
							}
					...
					lock (VirtualAllocLock)
					{
						if (virtualAlloc == null)
						{
							virtualAlloc = Is32BitSubsystem
								? new Func<IntPtr, ulong, uint, uint, IntPtr>((lpAddress, size, allocationTypeFlags, protectFlags) =>
								{
							...
							: new Func<IntPtr, ulong, uint, uint, IntPtr>(UnsafeNativeMethods.VirtualAlloc64);
						}
					}
				}
					...
					lock (SetProcessWorkingSetSizeLock)
					{
						if (setProcessWorkingSetSize == null)
						{
							setProcessWorkingSetSize = Is32BitSubsystem
								? new Func<IntPtr, ulong, ulong, uint, bool>((processHandle, minWorkingSetSize, maxWorkingSetSize, flags) =>
								{
								...
								: new Func<IntPtr, ulong, ulong, uint, bool>(UnsafeNativeMethods.SetProcessWorkingSetSizeEx64);
						}
					}
				}
					...
					lock (GetProcessWorkingSetSizeLock)
					{
						if (getProcessWorkingSetSize == null)
						{
							getProcessWorkingSetSize = Is32BitSubsystem
								? new GetProcessWorkingSetSizeExDelegate(GetProcessWorkingSetSizeEx32Wrapper)
								: new GetProcessWorkingSetSizeExDelegate(UnsafeNativeMethods.GetProcessWorkingSetSizeEx64);
						}
					}
				}
