...
namespace Isopoh.Cryptography.Argon2
{
	...
	internal sealed partial class Argon2
	{
		...
		private void Transformer_TransformBytes(IBlockTransformer transformer, byte[] value)
		{
			...
			transformer.TransformBytes(value);
		}
		...
		private SecureArray<byte> InitialHash()
		{
			...
			var transformer = bhash.CreateBlockTransformer();

			var value = new byte[4];
			Store32(value, this.config.Lanes);
			Transformer_TransformBytes(transformer, value);
			...
			{
				Transformer_TransformBytes(transformer, this.config.Password);
				if (this.config.ClearPassword)
				{
					SecureArray.SecureArray.Zero(this.config.Password);
				}
			}
			...
			{
				Transformer_TransformBytes(transformer, this.config.Salt);
			}
			...
			{
				Transformer_TransformBytes(transformer, this.config.Secret);
				if (this.config.ClearSecret)
				{
					SecureArray.SecureArray.Zero(this.config.Secret);
				}
			}
			...
			{
				Transformer_TransformBytes(transformer, this.config.AssociatedData);
			}
					...
					for (int sliceNumber = 0; sliceNumber < SyncPointCount; ++sliceNumber)
					{
						...
						for (; laneNumber < waitHandles.Length && laneNumber < this.config.Lanes; ++laneNumber)
						{
							ThreadPool.QueueUserWorkItem(
								...
								new FillState(new Position(passNumber, laneNumber, sliceNumber), (AutoResetEvent)waitHandles[laneNumber]));
						}
						...
						while (laneNumber < this.config.Lanes)
						{
							...
							ThreadPool.QueueUserWorkItem(
								...
								new FillState(new Position(passNumber, laneNumber, sliceNumber), (AutoResetEvent)waitHandles[i]));
							++laneNumber;
						}
		...
		private sealed class Position
		{
			...
			public Position(int pass, int lane, int slice, int index = 0)
			{
				Pass = pass;
				Lane = lane;
				Slice = slice;
